...
def load_transaction_data():
    ...
    try:
        with open(TRANSACTION_FILE, 'r') as file:
            return json.load(file)  # Expecting dict of lists
    except json.JSONDecodeError:
        print('Warning: File corrupted or empty. starting fresh')
        return {}

def save_transaction_data(data):
    with open(TRANSACTION_FILE, 'w') as file:  # overwrite file, not append
        json.dump(data, file, indent=4)
...
def create_account(data, data2):
    ...
    data[acc_num] = deets
    data2[acc_num] = []  # Initialize empty list for transactions
    save_data(data)
...
def add_transaction(data2, acc_num, deets):
    if acc_num not in data2:
        data2[acc_num] = []
    data2[acc_num].append(deets)
    save_transaction_data(data2)
    ...
    while pin_trials > 0:
        ...
        if hashpin(pin) == user['pin']:
            ...
            while True:
                ...
                if option == 1:
                    pin_confirm = input('Please enter your pin to confirm \n >')
                    if hashpin(pin_confirm) == user['pin']:
                    ...
                    deets = {
                        'details': 'Your money has been successfully deposited',
                        'amount': amount,
                        'Time': t
                    }

                    add_transaction(data2, acc_num, deets)
                    ...
                    if hashpin(trans_pin) == user['pin']:
                        ...
                        add_transaction(data2, acc_num, deets_sender)
                        ...
                        add_transaction(data2, dest_account, deets_receiver)
                    ...
                    if hashpin(pin_confirm) == user['pin']:
                        ...
                        t = str(datetime.now())
                        deets = {
                            'details': 'Your withdrawal was successful',
                            'amount': amount,
                            'Time': t
                        }
                    ...
                    pin_confirm = input('Please enter your pin to view transaction \n >')
                    if hashpin(pin_confirm) == user['pin']:
                        transactions = data2.get(acc_num, [])
                        if transactions:
                            print('Below is all transactions done on this account:')
